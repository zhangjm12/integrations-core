...
class Istio(OpenMetricsBaseCheck):
    ...
    ISTIOD_METRICS = {
        ...
        'pilot_conflict_outbound_listener_tcp_over_current_tcp': (
            'pilot.conflict.outbound_listener.tcp_over_current_tcp'
        ),
        ...
        'citadel_secret_controller_svc_acc_created_cert_count': (
            'citadel.secret_controller.svc_acc_created_cert_count'
        ),
        'citadel_secret_controller_svc_acc_deleted_cert_count': (
            'citadel.secret_controller.svc_acc_deleted_cert_count'
        ),
    ...
    def check(self, instance):
        ...
        istiod_endpoint = instance.get('istiod_endpoint')
        if istiod_endpoint:
            istiod_config = self.config_map[istiod_endpoint]

            self.process(istiod_config)
            processed = True
    ...
    def create_generic_instances(self, instances):
        ...
        for instance in instances:
            if 'istiod_endpoint' in instance:
                result.append(self._create_istiod_instance(instance))
    ...
    def _create_istiod_instance(self, instance):
        """
        Collect metrics from istiod deployment
        """

        endpoint = instance.get('istiod_endpoint')
        istiod_instance = deepcopy(instance)
        istiod_instance.update(
            {
                'namespace': 'istio',
                'prometheus_url': endpoint,
                'label_to_hostname': endpoint,
                'metrics': [self.ISTIOD_METRICS],
            }
        )
        return istiod_instance
